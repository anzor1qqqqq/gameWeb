...
const HeaderSearch: FC<IPropsHeaderSearch> = ({callback}): JSX.Element => {
    const [switchToInput, setSwitchToInput] = React.useState<boolean>(false);

    const stateValue = useSelector((state: IStateStore) => state);

    const switchStatus = React.useCallback(() => setSwitchToInput(prev => !prev), []);

    if (switchToInput) {
        return (
            <div className='header_search'>
                <div className='container_input_mobile'>
                    <InputSearch callback={switchStatus}/>
                    <button className='input_mobile_btn' onClick={switchStatus}><BtnClose/></button>
                </div>
            </div>
        );
    };
